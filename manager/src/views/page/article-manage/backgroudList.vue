    ...
    <template v-if="!selected">
      ...
      >
        <Form ref="form" :model="form" :label-width="100">
          ...
          <FormItem class="form-item-view-el" label="内容" prop="content">
            <editor
              ref="editor"
              openXss
...
import {
  ...
  updateArticle,
  seeArticle,
  updateArticleStatus, getWelcome, saveWelcome, getBackground, saveBackground,
} from "@/api/pages";
...
export default {
  ...
  methods: {
    ...
    getDataList(val) {
      if (val) this.form = {};
      this.loading = true;
      getBackground(this.searchForm).then((res) => {
        this.loading = false;
        if (res.success) {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          if (this.modalType === 0) {
            ...
            delete this.form.id;
            saveBackground(this.form).then((res) => {
              this.submitLoading = false;
              if (res.success) {
