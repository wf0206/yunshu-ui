      ...
      >
        <div class="base-info-item">
          ...
          <div class="form-item-view">
            ...
            <FormItem label="商品中文名称" prop="goodsName">
              <Input
                type="text"
            ...
            <FormItem label="商品UK名称" prop="goodsUkName">
              <Input
                type="text"
                v-model="baseInfoForm.goodsUkName"
                placeholder="商品UK名称"
                clearable
                style="width: 260px"
              />
            </FormItem>
            <FormItem label="商品MY名称" prop="goodsMyName">
              <Input
                type="text"
                v-model="baseInfoForm.goodsMyName"
                placeholder="商品MY名称"
                clearable
                style="width: 260px"
              />
            ...
            <FormItem label="商品uk卖点" prop="sellingUkPoint">
              <Input
                v-model="baseInfoForm.sellingUkPoint"
                type="textarea"
                :rows="4"
                style="width: 260px"
              />
            </FormItem>
            <FormItem label="商品my卖点" prop="sellingMyPoint">
              <Input
                v-model="baseInfoForm.sellingMyPoint"
                type="textarea"
                :rows="4"
                style="width: 260px"
              />
          ...
          <div class="form-item-view">
            <FormItem
              ...
              prop="goodsUnit"
            >
              <Select v-model="baseInfoForm.goodsUnit" style="width: 100px" :transfer="true">
                            ...
                            <Form :model="item" class="flex">
                              ...
                              >
                                ...
                                <AutoComplete
                                  ...
                                  style="width: 150px"
                                  :maxlength="30"
                                  placeholder="请输入uk规格项"
                                  @on-focus="changeSkuVals(item.name)"
                                  @on-change="
                                    skuValueChange(val.ukValue, index, item)
                                  "
                                >
                                </AutoComplete>
                                <AutoComplete
                                  ...
                                  style="width: 150px"
                                  :maxlength="30"
                                  placeholder="请输入my规格项"
                                  @on-focus="changeSkuVals(item.name)"
                                  @on-change="
                                    skuValueChange(val.myValue, index, item)
                                  "
                                >
                                </AutoComplete>
                                <Button
          ...
          <div class="form-item-view">
            ...
            >
              <editor
                ref="editorMY"
                openXss
                v-model="baseInfoForm.mobileMyIntro"
                :init="{ ...initEditor, height: '300px' }"
              ></editor>
            </FormItem>
            <FormItem
            ...
            >
              <editor
                ref="editorUK"
                openXss
                v-model="baseInfoForm.mobileUkIntro"
                :init="{ ...initEditor, height: '200px' }"
              ></editor>
            </FormItem>
          </div>
  ...
  methods: {
    ...
    GET_GoodsUnit() {
      API_GOODS.getGoodsUnitList(this.params).then((res) => {
        if (res.success) {
          this.goodsUnitList.push(...res.result.map((i) => i.name));
    ...
    renderGoodsDetailSku(skuList) {
      ...
      let specListTemp = []
      let allSpecListTemp = []
      if(skuList.length >0  && skuList[0].specList.length>=1){
        let value = skuList[0].specList[1].specValue
        specListTemp = JSON.parse(value)
        specListTemp.forEach(c=>{
          c.spec_values.forEach(u=>{
            let hj ={
              name : c.name,
              myName : c.myName,
              ukName : c.ukName,
      ...
      skuList.map((e) =>{
        let specList = []
        let names = e.goodsName.split(" ")
        e.specList.forEach((r)=>{
          if (r.specName === "images") {
            specList.push(r)
            names.forEach((v,i)=>{
              if(i>0){
                allSpecListTemp.forEach(w => {
                  if(w.value === v) {
                    let bn ={
                        specName:w.name,
                        specValue:w.value,
                        specUkName:w.ukName,
                        specUkValue:w.ukValue,
                        specMyName:w.myName,
                        specMyValue:w.myValue,
                    }
                    specList.push(bn)
                  }
                })
      ...
      skuList.map((e) => {
        ...
        e.specList.forEach((u) => {
          if (u.specName === "images") {
            ...
            sku[u.specUkName] = u.specUkValue;
            sku[u.specMyName] = u.specMyValue;
            if (
                ...
                spec_values: [
                  {
                    ...
                    name: u.specName,
                    value: u.specValue || "",
                    myValue: u.specMyValue || "",
                    ukValue: u.specUkValue || "",
                  },
                ],
              });
            } else {
    ...
    addSkuItem() {
      ...
      this.$set(this.skuInfo, this.skuInfo.length, {
        spec_values: [{ name: "规格名" + num, value: "",ukValue:"",myValue:"" }],
        name: "规格名" + num,
      });
      this.renderTableData(this.skuTableData);
    },
    ...
    editSkuItem() {
      this.skuInfo.forEach(item1 => {
        item1.spec_values.forEach(item =>{
      ...
      if (cloneTemp[0]) {
        ...
        cloneTemp[0].spec_values.forEach((specItem, index) => {
          ...
          if (skus && skus[index] && specItem.value !== "") {
            ...
            if(cloneTemp[0].ukName !== undefined && cloneTemp[0].ukName !== ""){
              obj[cloneTemp[0].ukName]  = specItem.ukValue
            }
            if(cloneTemp[0].myName !== undefined && cloneTemp[0].myName !== ""){
              obj[cloneTemp[0].myName]  = specItem.myValue
            }
            if (specItem.value !== "") {
            ...
            if(cloneTemp[0].ukName !== undefined && cloneTemp[0].ukName !== ""){
              obj1[cloneTemp[0].ukName]  = specItem.ukValue
            }
            if(cloneTemp[0].myName !== undefined && cloneTemp[0].myName !== ""){
              obj1[cloneTemp[0].myName]  = specItem.myValue
            }
            result.push(obj1);
          }
        });
    ...
    save() {
      ...
      this.$refs["baseInfoForm"].validate((valid) => {
        if (valid) {
          ...
          let skuInfoukNames = this.skuInfo.map((n) => n.ukName);
          let skuInfomyNames = this.skuInfo.map((n) => n.myName);
          submit.skuList = [];
          ...
          this.skuTableData.map((sku) => {
            ...
            if(!this.goodsId){
              for (let skuInfoName of skuInfoNames) {
                let name = skuInfoName + '.zh'
                let value =  sku[skuInfoName] + '.zh'
                skuCopy[name] = value;
              }
              for (let skuInfoName1 of skuInfoukNames) {
                let name = skuInfoName1 + '.uk'
                let value =  sku[skuInfoName1] + '.uk'
                skuCopy[name] = value;
              }
              for (let skuInfoName2 of skuInfomyNames) {
                let name = skuInfoName2+ '.my'
                let value =  sku[skuInfoName2] + '.my'
                skuCopy[name] = value;
              }
            }
            else {
              ...
              let j = 0;
              let k = 0;
              for (i = 0; i < skuInfoNames.length; i++) {
                let ar = i
                let cnname = skuInfoNames[ar]
                let cnvalue =  sku[cnname]
                skuCopy[cnname + '.zh'] = cnvalue + '.zh';
              }
              for (j = 0; j < skuInfoukNames.length; j++) {
                let ar = j
                let ukname = skuInfoukNames[ar]
                let ukvalue =  sku[ukname]
                skuCopy[ukname + '.uk'] = ukvalue + '.uk';
              }
              for (k = 0; k < skuInfomyNames.length; k++) {
                let ar = k
                let myname = skuInfomyNames[ar]
                let myvalue =  sku[myname]
                skuCopy[myname + '.my'] = myvalue + '.my';
              }
            }
            skuCopy["skuExt"] = this.skuInfo
